...
class NeuralSCA:
    ...
    def train(self, 
        ...
        if isinstance(traces, np.ndarray):
            if labels is None:
                raise ValueError("Labels required when traces is numpy array")
...
class LeakageSimulator:
    ...
    def _add_sbox_leakage(self, trace: np.ndarray, values: np.ndarray):
        ...
        hw = np.sum([(v >> i) & 1 for i in range(8)], axis=0) if len(values) > 0 else 0
    ...
    def _add_noise(self, trace: np.ndarray) -> np.ndarray:
        """Add realistic noise to trace."""
        # Gaussian noise
        gaussian_noise = np.random.normal(
            0, self.noise_params['gaussian_std'], len(trace)
        )
        
        # Pink noise (1/f noise)
        pink_noise = self._generate_pink_noise(len(trace))
        
        # Quantization noise
        quantization_levels = 2 ** self.noise_params['quantization_bits']
        trace_quantized = np.round(trace * quantization_levels) / quantization_levels
        
        return trace_quantized + gaussian_noise + pink_noise
    ...
    def _generate_pink_noise(self, length: int) -> np.ndarray:
        ...
        # 1/f characteristic
        fft_pink = fft_white / np.sqrt(np.abs(frequencies))
